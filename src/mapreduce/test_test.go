...
func TestBasic(t *testing.T) {
	...
	mr := setup()
	for i := 0; i < 2; i++ {
		go RunWorker(mr.MasterAddress, port( /*"worker"+strconv.Itoa(i)*/ i),
...
func TestOneFailure(t *testing.T) {
	...
	go RunWorker(mr.MasterAddress, port( /*"worker"+strconv.Itoa(0)*/ 0),
		MapFunc, ReduceFunc, 10)
	go RunWorker(mr.MasterAddress, port( /*"worker"+strconv.Itoa(1)*/ 1),
		MapFunc, ReduceFunc, -1)
	// Wait until MR is done
	<-mr.DoneChannel
	check(t, mr.file)
...
func TestManyFailures(t *testing.T) {
	...
	for !done {
		...
		default:
			...
			w := port( /*"worker" + strconv.Itoa(i)*/ i)
